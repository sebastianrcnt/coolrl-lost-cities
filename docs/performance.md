...
The following dated records and design notes were extracted from this
document on 2026-05-08 to keep `performance.md` focused on current-state
reference material. The split followed the AGENTS.md "Docs & Experiment
Workflow" routing rules:

- **Performance experiments (2026-05-07)** — `torch.compile` regression,
  AMP regression, GPU forward profiling, Option B interleaved traversal
  pass: `docs/archive/deep-cfr-performance-experiments-2026-05-07.md`.
- **Batched traversal inference design rationale (A vs B vs C)** —
  durable architecture note:
  `docs/research/batched-traversal-inference-decision.md`.
- **Post-A optimization calculus** — forward-looking sequencing
  recorded before Option A was benched:
  `docs/archive/post-a-optimization-calculus-2026-05-07.md`.
- **Option A bench result + structural-ceiling diagnosis** — why
  Option A regressed (sync-blocking policy boundary, not IPC plumbing)
